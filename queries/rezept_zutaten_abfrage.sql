...
SELECT 
    'ZUTAT-DETAILS' as TYP,
    r.REZEPTNR,
    z.BEZEICHNUNG as ZUTAT_NAME,
    CONCAT(rz.MENGE, ' ', rz.EINHEIT, ' - €', ROUND(z.NETTOPREIS * rz.MENGE, 2)) as DETAILS,
    NULL as ZUBEREITUNGSZEIT,
    NULL as ANZAHL_ZUTATEN,
    NULL as GESAMTKOSTEN,
    NULL as GESCHÄTZTE_GESAMTKALORIEN,
    NULL as GESCHÄTZTES_GESAMTPROTEIN,
    NULL as GESCHÄTZTE_GESAMTKOHLENHYDRATE
FROM REZEPT r
INNER JOIN REZEPTZUTAT rz ON r.REZEPTNR = rz.REZEPTNR
INNER JOIN ZUTAT z ON rz.ZUTATENNR = z.ZUTATENNR
WHERE r.REZEPTNR = 1  -- PARAMETER: Rezept-ID
ORDER BY TYP DESC, ZUTAT_NAME;
